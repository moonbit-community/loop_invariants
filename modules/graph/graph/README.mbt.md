...
- `modules/graph/graph/graph.mbt`: topological sort, Dijkstra, Union-Find,
  Bellman-Ford, Floyd-Warshall.
- `modules/graph/graph/tarjan.mbt`: Tarjan SCC with explicit stack reasoning.
- `modules/graph/graph/more_graph.mbt`: bipartite check, cycle detection, Kosaraju SCC,
  Prim MST, Kruskal MST, additional shortest-path variants.
- `modules/graph/graph/network_flow.mbt`: Edmonds-Karp, Dinic, min-cost max-flow,
...
- For performance-critical code, check the dedicated implementations in `modules/graph`.
