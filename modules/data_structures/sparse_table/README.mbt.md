...
test "sparse table range minimum" {
  ...
  debug_inspect(st.query(1, 4), content="1") // min of [2,4,7,1]
  debug_inspect(st.query(0, 2), content="2") // min of [5,2,4]
  debug_inspect(st.query(3, 3), content="7") // single element
}
```
